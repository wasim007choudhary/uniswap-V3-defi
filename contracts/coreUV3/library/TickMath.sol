...
library TickMath {
    ...
    function getSqrtPriceRatioAtTick(int24 tick) internal pure returns (uint160 sqrtPriceX96OfTheTick) {
        ...
        if (
            absTick > uint256(int256(MAX_VALID_TICK)) //ase abs will always be uint256 because of the above shit, we dont need to worry about the shit as max here = min as we removed that thing {
        ) {
